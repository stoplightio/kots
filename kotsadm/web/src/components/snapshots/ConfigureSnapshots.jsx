...
export default function ConfigureSnapshots(props) {
  const { configureSnapshotsModal, toggleConfigureModal, snapshotSettings, hideCheckVeleroButton, fetchSnapshotSettings, renderNotVeleroMessage, history } = props;

  return (
    <Modal
      isOpen={configureSnapshotsModal}
      shouldReturnFocusAfterClose={false}
      onRequestClose={() => { toggleConfigureModal(history); }}
      ariaHideApp={false}
      contentLabel="Modal"
    ...
    >
      <div className="Modal-body">
        ...
        <div className="flex justifyContent--flexStart u-marginTop--20">
          <button className="btn primary" onClick={() => { toggleConfigureModal(history); }}> Ok, got it! </button>
        </div>
      </div>
    </Modal>
  );
